...
parser.add_argument('-act','--activation', type=str, default='relu', help='which activation function(relu,softplus,leaky_relu,sigmoid)') # relu, softplus
...
if __name__ == '__main__':
  ...
  elif data_name == 'yahooanswers': bs = 250 #1000
  # elif data_name == 'newscategory': bs = 3000 #2000
  ...
  os.chdir(home_dir)
  save_dir = save_dir_no_bkp+"/SavedOutput/"+data_name+"/"+dtype+"/topics_"+str(args.num_topic)+"/ext"+str(ext)+"/qs_"+str(queryset)+"/lr_"+str(learning_rate)+"/bs_"+str(bs)+"/run_"+str(args.run) # +'/th_'+str(th)+
