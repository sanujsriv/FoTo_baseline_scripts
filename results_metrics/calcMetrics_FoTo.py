...
def cal_AUPR_f(num_docs,num_keyword,num_coordinate,relv_docs_in_vis_idx,x_list,query_center):
...
# data_names=['bbc'] # 'nfcorpus', 'opinions_twitter','searchsnippet','yahooanswers'
...
parser = argparse.ArgumentParser(description='FoTo-Metrics')
...
X = all_results['X']
labels = all_results['labels_list']
...
query_center = all_results['query_center']
...
map_true_ranking, minDist_x_q, aupr = cal_AUPR_f(len(X),len(keywords_as_labels),2,relv_docs_in_vis_idx,X,query_center)
os.chdir(model_results_dir+save_dir_no_bkp)


all_metrics['X'] = all_results['X']
all_metrics['labels'] = all_results['labels_list']
all_metrics['query_center'] =all_results['query_center']
