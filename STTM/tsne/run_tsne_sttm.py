...
STTM_results_dir = '/home/student_no_backup/sakumar/'+paper+'/STTM'
STTM_data_dir= '/home/grad16/sakumar/'+paper+'/dataset/content/STTM_data'

# model_names = ['PTM']
# data_names=['yahooanswers'] # 'nfcorpus', 'opinions_twitter'
# num_topics = ['10','20','30','40','50']
# qss = ['1','2']
# fracs = ['0.05','0.1','0.15','0.2']
# fracs = ['0.2']
# runs = [1,2,3,4,5]#,2] # 1,2
...
save_dir_no_bkp = '/SavedOutput/'+m_name+"/"+data_name+"/short/topic_"+num_topic+"/qs"+qs+"/run_"+str(r)+'/results'
os.chdir(STTM_results_dir+save_dir_no_bkp)
with open(m_name+"_"+data_name+"_"+"numtopic_"+num_topic+"_run_"+str(r)+"_qs_"+qs+".theta") as f:
          content = f.read()
          content = content.split('\n')[:-1 ]
          theta = np.asarray([np.asarray(x.split(' ')[:-1]).astype('float') for x in content])
with open(m_name+"_"+data_name+"_"+"numtopic_"+num_topic+"_run_"+str(r)+"_qs_"+qs+".topWords") as f:
          topicwords = f.read()
          topicwords = topicwords.split('\n')
          topicwords = [x.strip() for x in topicwords][:-1]
          topics_wordlist = [x.split(' ') for x in topicwords]
with open(m_name+"_"+data_name+"_"+"numtopic_"+num_topic+"_run_"+str(r)+"_qs_"+qs+".paras") as f:
          runtime = f.read()
...
print(os.getcwd())
compressed_pickle(all_results,m_name+"_metrics_"+data_name+"_"+"numtopic_"+num_topic+"_run_"+str(r)+"_qs_"+qs)
