...
def get_cosine_sum_topics(all_topics,m_beta,topwords,id_vocab,embeddings,keywords):
  all_topics = get_topwords(m_beta,id_vocab,topwords=topwords)
  ...
  for topics in topics_wordlist:
    ...
    np_topics_wordsScores_FoTo.append(np_topics_wordsScores)
  
  sum_avg_cos = []
  for np_topics_wordsScores in np_topics_wordsScores_FoTo:
    # print(torch.max(np_topics_wordsScores,0).values)
    sum_avg_cos.append((np_topics_wordsScores.mean(-1)).sum(-1).item())
    # sorted_topic_scores = np.sort(sum_avg_cos)[::-1]
    # sorted_topic_indx = np.argsort(sum_avg_cos)[::-1]
  sum_avg_cos = np.array(sum_avg_cos)
  return sum_avg_cos

# def sum_of_avg_cosine():
#   np_topics_wordsScores_FoTo = get_cosine_sum_topics(FoTo_topics,foto_beta,topwords=20)
#   # topk=
#   sum_avg_cos = []
#   for np_topics_wordsScores in np_topics_wordsScores_FoTo:
#     # print(torch.max(np_topics_wordsScores,0).values)
...
def plot_relv_irrelv_docs(model_name,filename,zx1,zx2,l1,l2, zphi, query_center,query_words,hv_qwords,keywords,lim,contour='No'):
